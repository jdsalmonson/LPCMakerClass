/*
 Stepper: #4 iteraton of the program: This program will move a stepper motor with linear displacement
...
Stepper motor(48, in1Pin, in2Pin, in3Pin, in4Pin);  //48stp/rev into gearing...
                                                   // steps/rev, pin # defines follow // here created new class "motor"???
       float r = 2.0;   //defines radius to feed to StepFuncMath
...
void loop() {
  if (Serial.available())  {                          //wait for input, until given
     ...
     if (lengths != 0.0)  {
       ...
       steps = stepFunction(lengths, r, pi);          //actually passes vars to StepFuncMath
...
}                                  //break out of this, you're either in a function, or you branched to oblivion (you broke it).

float reverseMath(int stepstaken) {
  float result;
  result = ((r*2.0*pi*stepstaken)/2048.0);
  return result;
}

 float stepFunction(float lengths, float r, float pi) {    //StepFuncMath
    int result;
    result = (lengths * 2048.0)/(r*2.0*pi);    //product of 2048 lengths, divided by Two_Pi_R
